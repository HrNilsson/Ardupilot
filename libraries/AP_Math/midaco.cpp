...
/* Increase the size if PMAX and PXM if necessary (IFLAG: 801 or 802) */
   ...
   static int  PMAX = 100;
   static double    a_global[100]; /* Array a[] must be of length PMAX */
   static double    b_global[100]; /* Array b[] must be of length PMAX */
 ...
 if (*p * *m + 1 > PXM) {
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    ...
    jfk_(p, n, ni, m, me, &x[1], &f[1], gm, &xl[1], &xu[1], iflag, istop, &
      param[1], &rw[1], lrw, &iw[1], liw, a_global, b_global, key);
    return 0;
} /* midaco_ */
...
/* Computing MAX */
    d__2 = *b * 30.;
    i__2 = i_dnnt(&d__2);
    ret_val = g[(0 + (0 + ((maxM(i__1,1) - 1) << 3))) / 8] * i__[(0 + (0 + ((maxM(
      i__2,1) - 1) << 3))) / 8];
    return ret_val;
} /* i04_ */
  ...
  doublereal *i68, doublereal *i48, integer *i36)
{
  ...
  for (i__ = 1; i__ <= i__2; ++i__) {
      i4[*i19 + (j - 1) * *m + i__ - 1] = 3.402823e38; // 1.0666e90; MAGIC NUMBERS
  }
  i4[*i40 + j - 1] = 3.402823e38;//1.0777e90;
  i4[*i14 + j - 1] = 3.402823e38;//1.0888e90;
  i4[*i11 + j - 1] = 3.402823e38;//1.0999e90;
    }
    return 0;
} /* i09_ */
